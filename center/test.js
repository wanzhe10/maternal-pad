...
! function(t, e) {
	...
	function Ct(t, e) {
		var i = (parseFloat(t[0]) 60 + 360) 60 / 360,
			n = bt(t[1]),
			o = bt(t[2]),
			a = o <= .5 ? o * (n + 1) : o + n - o * n,
			r = 2 * o - a;
	...
	function qo(t, e, i) {
		...
		return t = t.replace("MM", Yo(r, 2)).replace("M", r).replace("yyyy", a).replace("yy", a 0).replace("dd", Yo(s, 2)).replace("d", s).replace("hh", Yo(l, 2)).replace("h", l).replace("mm", Yo(u, 2)).replace("m", u).replace("ss", Yo(h, 2)).replace("s", h).replace("SSS", Yo(c, 3))
	}
	...
	var TC = In.extend({
		...
		AC = (gr.extend({
			...
			_renderMain: function(t, e, i, n, o) {
				for (var a = this.group, r = t.getModel("axisLine").getModel("lineStyle"), s = t.get("clockwise"), l = -t.get("startAngle") / 180 * Math.PI, u = -t.get("endAngle") / 180 * Math.PI, h = (u - l) Ü, c = l, d = r.get("width"), f = 0; f < n.length; f++) {
