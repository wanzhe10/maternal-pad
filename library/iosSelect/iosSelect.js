 ...
 * @param {string=} options.oneLevelTitle 第一级选中Title
 * @param {string=} options.twoLevelTitle 第二级选中Title
 * @param {string=} options.threeLevelTitle 第三级选中Title
 * @param {string=} options.fourLevelTitle 第四级选中Title
 * @param {string=} options.fiveLevelTitle 第五级选中Title
 * @param {string=} options.sixLevelTitle 第六级选中Title
...
(function () {
	...
	function IosSelect(level, data, options) {
		...
		if ([1, 2, 3, 4, 5, 6, 7].indexOf(level) == -1) {
			throw new RangeError('the level parameter must be one of 1,2,3,4,5,6,7!');
			return;
		}
		this.data = data;
		this.level = level || 1;
		...
		if (this.level === 1) {
			...
			this.typeBox = 'seven-level-box';
		}
		this.title = options.title || '';
		this.oneLevelTitle = options.oneLevelTitle || '';
		console.log(options)
		this.twoLevelTitle = options.twoLevelTitle || '';
		this.threeLevelTitle = options.threeLevelTitle || '';
		this.fourLevelTitle = options.fourLevelTitle || '';
		this.fiveLevelTitle = options.fiveLevelTitle || '';
		this.sixLevelTitle = options.sixLevelTitle || '';
		this.sevenLevelTitle = options.sevenLevelTitle || '';
	...
	IosSelect.prototype = {
		init: function () {
			this.initLayer();
			this.setLevelData(1, this.options.oneLevelId, this.options.twoLevelId, this.options.threeLevelId, this.options.fourLevelId, this.options.fiveLevelId, this.options.sixLevelId, this.options.sevenLevelId);
		},
		initLayer: function () {
			var self = this;
			...
			this.iosSelectLayer.el.querySelector('#select-one-level-title').innerHTML = this.oneLevelTitle;
			this.iosSelectLayer.el.querySelector('#select-two-level-title').innerHTML = this.twoLevelTitle;
			this.iosSelectLayer.el.querySelector('#select-three-level-title').innerHTML = this.threeLevelTitle;
			this.iosSelectLayer.el.querySelector('#select-four-level-title').innerHTML = this.fourLevelTitle;
			this.iosSelectLayer.el.querySelector('#select-five-level-title').innerHTML = this.fiveLevelTitle;
			this.iosSelectLayer.el.querySelector('#select-six-level-title').innerHTML = this.sixLevelTitle;
			this.iosSelectLayer.el.querySelector('#select-seven-level-title').innerHTML = this.sevenLevelTitle;
			...
			this.sevenLevelContainDom = this.iosSelectLayer.el.querySelector('#sevenLevelContain');
			...
			this.sevenLevelUlContainDom = this.iosSelectLayer.el.querySelector('.select-seven-level');
			...
			if (this.level >= 7) {
				this.sevenLevelContainDom.style.height = this.options.itemHeight * this.options.itemShowCount + this.options.cssUnit;
				this.scrollSeven = new IScrollForIosSelect('#sevenLevelContain', {
					probeType: 3,
					bounce: false
				});
				this.setScorllEvent(this.scrollSeven, 7);
			}

			// 确认
			this.selectBtnDom = this.iosSelectLayer.el.querySelector('.sure');
			this.selectBtnDom.addEventListener('click', function (e) {
				self.options.callback && self.options.callback(self.selectOneObj, self.selectTwoObj, self.selectThreeObj, self.selectFourObj, self.selectFiveObj, self.selectSixObj, self.selectSevenObj);
			});
		},
		mapKeyByIndex: function (index) {
			...
			if (index === 2) {
				...
				map = {
					...
					relation: self.options.sixSevenRelation
				};
			}
			else if (index === 7) {
				map = {
					index: 7,
					levelContain: self.sevenLevelContainDom,
					relation: 0
				};
			}
			// else if (index === 7) {
			...
			scrollInstance.on('scrollEnd', function () {
				...
				if (self.level > index) {
					if ((mapKey.relation === 1 && iosSelectUtil.isArray(self.data[index])) || iosSelectUtil.isFunction(self.data[index])) {
						self.setLevelData(index + 1, self.selectOneObj.id, self.selectTwoObj.id, self.selectThreeObj.id, self.selectFourObj.id, self.selectFiveObj.id, self.selectSixObj.id, self.selectSevenObj.id);
					}
				}
			});
		...
		mapRenderByIndex: function (index) {
			...
			if (index === 2) {
				...
				map = {
					index: 7,
					relation: self.options.sixSevenRelation,
					levelUlContainDom: self.sevenLevelUlContainDom,
					scrollInstance: self.scrollSeven,
					levelContainDom: self.sevenLevelContainDom
				};
			}
			return map;
		},
		...
		setLevelData: function (index, oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, sixLevelId, sevenLevelId) {
			if (iosSelectUtil.isArray(this.data[index - 1])) {
				...
				this.renderLevel(index, oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, sixLevelId, sevenLevelId, levelData);
			}
		...
		renderLevel: function (index, oneLevelId, twoLevelId, threeLevelId, fourLevelId, fiveLevelId, sixLevelId, sevenLevelId, levelData) {
			var plast = 0;
			...
			if (this.level > index) {
				this.setLevelData(index + 1, this.selectOneObj.id, this.selectTwoObj.id, this.selectThreeObj.id, this.selectFourObj.id, this.selectFiveObj.id, this.selectSixObj.id, this.selectSevenObj.id);
			}
		},
