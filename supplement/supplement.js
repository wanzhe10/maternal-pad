 ...
 * @Last Modified time: 2018-10-29 11:08:03
 */
$(function () {
    var smokingRadio = ''; // 支烟
    var poison = 0; //接触毒物
    ...
    HttpRequstForPost(httpUrl.essentialInquire, 'json', {
        ...
        if (data.status == 20200) {
            ...
            $('.pregnantName').val(data.name);
            $('.pregnantFN').val(data.number);
            $('.pregnantPhone').val(data.telephone);
            $('.pregnantIdCard').val(data.idCard);
            $('.idCardType').val(data.idCardType);
            $('.pregnantDateBirth').val(data.birthdayDate);
            $('.pregnantMarriageAge').val(data.marryAge);
            $('.birth_age').html(data.age);
            if (data.sex == 1) {
                $('.birth_sex').html('女');
            } else {
                $('.birth_sex').html('男');
            }
            $('.pregnantWeight').val(data.lastWeight);
            $('.marryType').val(data.marryType);
            $('.marryCheck').val(data.marryCheck);
            $('.contraception').val(data.contraception);
            $('.education').val(data.education);
            $('.nation').val(data.nation);
            $('.job').val(data.job);
            $('.work').val(data.jobCompanyName);
            $('.areaInput').val(data.idCardAddressProvince + data.idCardAddressCity + data.idCardAddressCounty);
            $('.areaInput2').val(data.newAddressProvince + data.newAddressCity + data.newAddressCounty);
            $('.presentAddress').val(data.newAddressRemarks);
            $('.womenInformationLi1').attr('womenidentifying', 1);
    ...
    HttpRequstForPost(httpUrl.mateNewsInquire, 'json', {
        ...
        if (data.status == 20200) {
            ...
            $('.spouseName').val(data.name);
            $('.spouseIdCardType').val(data.idCardType);
            $('.spouseIdCard').val(data.idCard);
            $('.spouseAge').val(data.age);
            $('.spousePhone').val(data.telephone);
            $('.healthType').val(data.healthType);
            $('.spouseEducation').val(data.education);
            $('.spouseJob').val(data.job);
            $('.spouseMarriageAge').val(data.marryAge);
            $('.spouseMarryType').val(data.marryType);
            $('.spouseMarryCheck').val(data.marryCheck);
            if (data.smoke == '0') {
                $("input[name=smoking][value=0]").attr("checked", data.smoke == 0 ? true : false);
            } else {
                $("input[name=smoking][value=1]").attr("checked", data.smoke > 0 ? true : false);
                $('.cigaretteNUm').val(data.smoke).css({ 'background': '#fff' }).removeAttr('readonly');
            }
            if (data.drink == '0') {
                $("input[name=drink][value=0]").attr("checked", data.drink == 0 ? true : false);
            } else if ((data.drink == '1')) {
                $("input[name=drink][value=1]").attr("checked", data.drink == 1 ? true : false);
            } else {
                $("input[name=drink][value=2]").attr("checked", data.drink == 2 ? true : false);
            }
            if (data.patientHistory == '') {
                $("input[name=history1][value='否']").attr("checked", data.patientHistory == '' ? true : false);

            } else {
                $("input[name=history1][value='是']").attr("checked", !data.patientHistory == '' ? true : false);
                $('.inquire').removeAttr("readonly").css("background", "#fff");
                var patientHistory = data.patientHistory;
                var mateTemp = patientHistory.split('、');
                var _html = '';
                if (patientHistory.indexOf("、") >= 0) {
                    for (var i = 0; i < mateTemp.length - 1; i++) {
                        _html += '<div>\
                                    <span class="patientHistory">'+ mateTemp[i] + '</span>\
                                    <i class="deteleIcon"></i>\
                                    </div>'
                    }
                } else{
                    _html += '<div>\
                                    <span class="patientHistory">'+ patientHistory + '</span>\
                                    <i class="deteleIcon"></i>\
                                    </div>'
                }
                $('.supeDisease').append(_html);
            }
            $('.areaInput3').val(data.newAddressProvince + data.newAddressCity + data.newAddressCounty);
            $('.spouseSite').val(data.newAddressRemarks);
            layui.use('form', function () {
                var form = layui.form;
                form.render();
            });
    ...
    HttpRequstForPost(httpUrl.pregnantWomenCheck, 'json', {
        ...
        if (data.status == 20200) {
            ...
            $('.firstCheckDate').val(data.firstCheckDate);
            ...
            if (data.lastMenstruation == '') {
                ...
                $('.lastMenstruation').val(data.lastMenstruation);
                // 末次月经
                var newDate = new Date().getTime();
                var menstruationDate = $('.lastMenstruation').val();
                var oldDateTime = new Date(menstruationDate.replace(/-/g, '/'));
                var oldDate = oldDateTime.getTime();
                // console.log(oldDate)
            ...
            if (data.newAgeOfMenarche != null && data.newAgeOfMenarche != "") {
                $('.newAgeOfMenarche').val(data.newAgeOfMenarche);
            }
            if (data.newAgeOfMenarcheDay != null && data.newAgeOfMenarcheDay != "") {
                $('.newAgeOfMenarcheDay').val(data.newAgeOfMenarcheDay);
            }
            $('.newAgeOfMenarche').attr('weeks', data.newAgeOfMenarche);
            $('.menstrualHistoryAge').val(data.menstrualHistoryAge);
            $('.menstrualHistoryDay').val(data.menstrualHistoryDay);
            $('.pregnancyNum').val(data.pregnancyNumber);
            $('.parturitionFrontPharmacy').val(data.parturitionFrontPharmacy);
            $('.ketosis').val(data.ketosis);
            $('.morningSickness').val(data.morningSickness);
            $('.animalContact').val(data.animalContact);
            $('.contactRadioactiveRays').val(data.contactRadioactiveRays);
            if (data.contactRadioactiveRays == '1') {
                $('.contactRadioactiveRaysDate').val(data.contactRadioactiveRaysDate)
            } else {
                $('.contactRadioactiveRaysDate').val('').css('background', '#ccc').attr('disabled', 'disabled');
            }
            if (data.contactToxic == 0) {
                $("input[name=contact][value='0']").attr("checked", data.contactToxic == '0' ? true : false);
            } else {
                $("input[name=contact][value='1']").attr("checked", data.contactToxic == '1' ? true : false);
            }
            // 病毒感染？？？？？？？？
            if (data.virusInfection == 0) {
                $("input[name=virus][value='0']").attr("checked", data.virusInfection == '0' ? true : false);
            } else {
                $("input[name=virus][value='1']").attr("checked", data.virusInfection == '1' ? true : false);
                var virusInfectionOther = data.virusInfectionOther;
                var virus = virusInfectionOther.split('、');
                var _arr = [];
                for (var j = 0; j < $(".chunks").length; j++) {
                    _arr.push($('.chunks').eq(j).attr('value'))
                }
                for (var i = 0; i < virus.length - 1; i++) {
                    if (_arr.indexOf(virus[i]) != -1) {
                    $(".chunks").eq(_arr.indexOf(virus[i])).addClass("active");
                    }
                }
            }
            ...
            if (data.contactToxic == '0') {
                ...
                $("input[name=contact][value='1']").attr("checked", !data.contactToxic == '' ? true : false);
                $('.contactToxicName').removeClass('bgcC').addClass('bgcF');
                $('.contactToxicDate ').removeClass('bgcC').addClass('bgcF').val(data.contactToxicDate);
                var contactToxicName = data.contactToxicName;
                var contactemps = contactToxicName.split('++');
                var _html = '';
                for (var i = 0; i < contactemps.length - 1; i++) {
                    _html += '<div>\
                                <span class="contactPoison">' + contactemps[i] + '</span>\
                                <i class="deteleIcon"></i>\
                            </div>'
                }
            ...
            if (data.familyHistory == '') {
                $("input[name=history2][value='否']").attr("checked", data.familyHistory == '' ? true : false);
            } else {
                $("input[name=history2][value='是']").attr("checked", !data.familyHistory == '' ? true : false);
                $('.recordInquire').removeClass('bgcC').addClass('bgcF');
                var familyHistory = data.familyHistory;
                var temps = familyHistory.split('、');
                var _html = '';
                if (familyHistory.indexOf("、") >= 0) {
                    for (var i = 0; i < temps.length - 1; i++) {
                        _html += '<div>\
                                                    <span class="recordPatientHistory">' + temps[i] + '</span>\
                                                    <i class="deteleIcon"></i>\
                                                </div>'
                    }
                }else{
                    _html += '<div>\
                                                    <span class="recordPatientHistory">' + temps + '</span>\
                                                    <i class="deteleIcon"></i>\
                                                </div>'
                }
               
                $('.disease').append(_html);
            }
            //  现病史
            if (data.nowHistory == '') {
                $("input[name=presentIllness][value='否']").attr("checked", data.nowHistory == '' ? true : false);
            } else {
                $("input[name=presentIllness][value='是']").attr("checked", !data.nowHistory == '' ? true : false);
                $('.presentrecordInquire').removeClass('bgcC').addClass('bgcF');
                var nowHistory = data.nowHistory;
                var newtemps = nowHistory.split('、');
               
                var _nowHistory = '';
                if (nowHistory.indexOf("、") >= 0) {
                    for (var i = 0; i < newtemps.length - 1; i++) {
                        _nowHistory += '<div>\
                                                    <span class="presentIllness">' + newtemps[i] + '</span>\
                                                    <i class="deteleIcon"></i>\
                                                </div>'
                    }
                }else{
                    _nowHistory += '<div>\
                                    <span class="presentIllness">' + newtemps + '</span>\
                                    <i class="deteleIcon"></i>\
                                </div>'
                }
                $('.presentdisease').append(_nowHistory);
            }
            layui.use('form', function () {
                var form = layui.form;
                form.render();
            ...
            // 孕产史
            if (data.pregnancyNumber != null && data.pregnancyNumber != 0) {
                var historyList = data.patientParturitionDetailHistoryBeanList;
                if (historyList == null || historyList.length == 0) {
                    addHistoryWithNum(data.pregnancyNumber);
                    $('.degreeAffirm').hide();
                } else {
                    var _history = '';
                    var tempHistory;
                    for (var i = 0; i < historyList.length; i++) {
                        tempHistory = historyList[i];
                        _history += name(tempHistory);
                    }
                    $(".kang").html(_history);
    ...
    // 怀孕时封装函数
    function name(tempHistory) {
        var tempStr;
        var productionDate = tempHistory.productionDate;
        var proTime = productionDate.substring(0, 7)
        tempStr += "<tr name = '" + tempHistory.id + "' number = '" + tempHistory.number + "'>\
                        <td class='number inputWire'>"+ tempHistory.number + "</td>"
        if (tempHistory.pregnantType == 0) {
            tempStr += "<td class='ageOfMenarche'>早产</td>"
        } else if (tempHistory.pregnantType == 1) {
            tempStr += "<td class='ageOfMenarche'>足月妊娠</td>"
        } else {
            tempStr += "<td class='ageOfMenarche'>过期妊娠</td>"
        }
        tempStr += "<td class = 'productionDate inputWire'>" + proTime + "</td>\
                         <td class='productionOfAge inputWire'>"+ tempHistory.productionOfAge + "\
                        </td>"
        if (tempHistory.productionAbortion == 0) {
            tempStr += "<td class='productionAbortion'>自然</td>"
        } else {
            tempStr += "<td class='productionAbortion'>剖宫产</td>"
        }
        if (tempHistory.babySex == 0) {
            tempStr += "<td class='babySex'>男</td>"
        } else {
            tempStr += "<td class='babySex'>女</td>"
        }
        if (tempHistory.babyHealthType == 0) {
            tempStr += "<td class='babyHealthType'>健康</td>"
        } else {
            tempStr += "<td class='babyHealthType'>死亡</td>"
        }
        tempStr += " <td class='inputWire'>\
                                                <input type='text'  value='"+ tempHistory.remarks + "'class = 'remarks'>\
                                    </td>\
                    </tr>"
        return tempStr
    }
    //  体格检查  查询  ;
    $('.newRecordLi2 ').attr('id', recordData.healthCheckId);
    if ($('.newRecordLi2 ').attr('id') !== '') {
        HttpRequstForPost(httpUrl.findById, 'json', {
            ...
            // console.log(data)
            if (data.status == 20200) {
                $('.hyperpiesia').val(data.baseBloodPressureHigh);
                $('.lowTension').val(data.baseBloodPressureLow);
                $('.height').val(data.baseHeight);
                $('.weight').val(data.baseWeight);
                $('.assayUrineProtein').val(data.assayUrineProtein);
                $('.assayHemoglobin').val(data.assayHemoglobin);
                $('.assayBloodPlatelet').val(data.assayBloodPlatelet);
                $('.obstetricsHeight').val(data.obstetricsHeight);
                $('.obstetricsAbdominalGirth').val(data.obstetricsAbdominalGirth);
                $('.obstetricsFetalHeart').val(data.obstetricsFetalHeart);
                $('.obstetricsTransversePelvicDiameter').val(data.obstetricsTransversePelvicDiameter);
                $('.baseHeartRate').val(data.baseHeartRate);
                $('.baseLung').val(data.baseLung);
                $('.baseAbdomenLiver').val(data.baseAbdomenLiver);
                $('.baseAbdomenSpleen').val(data.baseAbdomenSpleen);
                $('.baseSpinalLimbsDeformity').val(data.baseSpinalLimbsDeformity);
                $('.baseBreasts').val(data.baseBreasts);
                $('.baseNipple').val(data.baseNipple);
                $('.baseSpinalLimbsEdema').val(data.baseSpinalLimbsEdema);
                $('.obstetricsVulva').val(data.obstetricsVulva);
                $('.obstetricsVagina').val(data.obstetricsVagina);
                $('.obstetricsCervix').val(data.obstetricsCervix);
                $('.obstetricsCorpus').val(data.obstetricsCorpus);
                $('.obstetricsPairsAttachment').val(data.obstetricsPairsAttachment);
                $('.assayBloodType').val(data.assayBloodType);
                $('.obstetricsFirstDew').val(data.obstetricsFirstDew);
                $('.obstetricsPlacental').val(data.obstetricsPlacental);
                $('.primaryDiagnosis').val(data.primaryDiagnosis);
                $('.disposal').val(data.disposal);
                layui.use('form', function () {
                    var form = layui.form;
                    form.render();
                });
    ...
    layui.use(['layer', 'form', 'element', 'laydate'], function () {
        ...
        $('.contactToxicDate').click(function () {
            $(this).removeClass('bgcC').addClass('bgcF');
            $("input[name=contact][value='0']").removeAttr("checked");
            $("input[name=contact][value='1']").prop("checked", true);
            form.render()
        ...
        form.on('select(pregnancyNum)', function (data) {
            var pregnantNumber = $('.pregnancyNum option:selected').attr('value');
            addHistoryWithNum(pregnantNumber);
        });
    });
    ...
    var tempPregnancyNum = 0;
        // 根据numb添加孕产信息-孕产史
        function addHistoryWithNum(nuumber) {
            var i = tempPregnancyNum * 1;
            if (tempPregnancyNum - nuumber > 0) {
                while (i - nuumber > 0) {
    ...
    $('.hideBox').on('click', 'li', function () {
        ...
        $('.spouse_box3_bottom .supeDisease').append(_html);
        $('.hideBox').hide();
        $('.inquire').val('');
    });
    // 家族史点击叉号删除块
    $('.supeDisease').on('click', '.deteleIcon', function () {
    ...
    $('.nextStepOne').click(function () {
        // console.log($(".areaInput3").attr('selectoneobj') + $(".areaInput3").attr('selecttwoobj') + $(".areaInput3").attr('selectthreeobj'))
    ...
    $('.nextStep2').click(function () {
        ...
        if ($('.recordInquire').hasClass('bgcC')) {
            ...
            for (var i = 0; i < $('.recordPatientHistory').length; i++) {
                ...
                //console.log(recordPatientHistory)
            }
        }
        //     收集现病史div内容
        var presentIllness = '';
        ...
        if (!$('.firstCheckDate').val()) { //初诊日期
            ...
            $('.reminder').html('请输入周期');
            $('html,body').addClass('noscroll');
        } else if (historyListArr.length != $('.pregnancyNum').val()) { //家族史
            $('.shade').show();
            $('.verifyAlert').show();
            $('.reminder').html('孕产史');
            $('html,body').addClass('noscroll');
            ...
            if (!$('.newRecordLi1').attr('id')) {
                HttpRequstForPost(httpUrl.pregnancyInsertForFiling, 'json', {
                    ...
                    "newAgeOfMenarcheDay": $('.newAgeOfMenarcheDay').val(), //现孕周 - 天
    ...
    $('.nextStepTwo').click(function () {
        ...
        if ($('.hyperpiesia').val().length == 0) { //高压
            ...
            $('html,body').addClass('noscroll');
        } else if ($('.assayHemoglobin').val() < 80 || $('.assayHemoglobin').val() > 200) { //血红蛋白取值范围
            layer.msg('血红蛋白取值范围 80 ~ 200， 请修改')
            ...
            $('html,body').addClass('noscroll');
        } else if ($('.obstetricsFetalHeart').val() < 90 || $('.obstetricsFetalHeart').val() > 200) { //胎心率取值范围
            layer.msg('胎心率取值范围 90 ~ 200， 请修改')
            ...
            if (!$('.newRecordLi2').attr('id')) {
                ...
                HttpRequstForPost(httpUrl.physiqueInsertForFiling, 'json', {
                    ...
                    if (data.status == 20200) {
                        recordData.healthCheckId = data.id;
                        localStorage.setItem('maternalrecordData', JSON.stringify(recordData));
